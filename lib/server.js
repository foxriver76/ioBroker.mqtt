...
function MQTTServer(adapter, states) {
            ...
            setImmediate(() => {
                ...
                for (const clientId in persistentSessions) {
                    if (Object.prototype.hasOwnProperty.call(persistentSessions, clientId) && !clients[clientId]) {
                        (function (_clientId) {
                            getMqttMessage(persistentSessions[_clientId], id, state, adapter.config.defaultQoS, true, (err, message) => {
                                message && persistentSessions[_clientId] && persistentSessions[_clientId].messages.push(message);
                            });
                        })(clientId);
                    }
                }
    ...
    /**
     * Ensures object is created for given topic
     * @param {string} id - id of the object
     * @param {string} topic - topic of the object
    ...
    async function checkObject(id, topic) {
        ...
        const obj = {
            common: {
                ...
                type:  topic2id[topic].message !== undefined ? typeof topic2id[topic].message : 'string' // we assume that this is the correct type
            },
            native: {
                topic: topic
            },
        ...
        adapter.log.debug(`Update object for topic: ${topic}[ID: ${id}]`);
        try {
            // extend the object because on mqtt topics, the type could change
            // TODO: if this still produces problems, we should make the type mixed
            await adapter.extendForeignObjectAsync(id, obj, {preserve: {common: ['name']}});
        } catch (e) {
            adapter.log.error(`Error updating object "${id}": ${e.message}`);
        }
        topic2id[topic].id = id;
        obj._id = id;
        return obj;
    ...
    async function receivedTopic(packet, client, cb) {
        let   isAck   = true;
        ...
        if (!topic2id[topic]) {
            ...
            processTopic(obj._id, topic, message, qos, retain, isAck, obj, client);
            if (cb) {
                cb();
                cb = null;
            }
